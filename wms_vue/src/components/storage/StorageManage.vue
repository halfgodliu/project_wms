<template>
  <div>
    ...
    <div style="margin-bottom: 5px">
      <!--根据根据名字模糊查询-->
      <el-input v-model="queryName" placeholder="请输入仓库名" @keyup.enter.native="loadPost"
                style="width: 200px"
    ...
    <el-table :data="tableData"
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          ...
          <el-popconfirm title="确定删除该仓库吗？" style="margin-left: 10px"
...
<script>
  export default {
    ...
    methods:{
      ...
      doSave(){//将新增数据发送到后端保存
        this.$axios.post(this.$httpUrl+'/storage/insert',this.form).then(res=>res.data).then(res=>{
          if (res.code==200){//先判断数据是否获取成功
            this.$message({//成功后消息提示成功
              message: '添加成功！！！',
              type: 'success'
